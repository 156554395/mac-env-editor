export class EnvManager {
    ...
    updateEnvironmentVariable(index, key, value) {
        if (index >= 0 && index < this.envVars.length) {
    ...
    searchEnvironmentVariables(query) {
        const lowerQuery = query.toLowerCase();
        return this.envVars.filter(env => env.key.toLowerCase().includes(lowerQuery) ||
            env.value.toLowerCase().includes(lowerQuery));
    }
    hasChanges(originalEnvVars) {
