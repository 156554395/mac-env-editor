...
export class SecurityManager {
    ...
    async createBackup(configPath, content) {
        try {
    ...
    async validateConfigFile(configPath, content) {
        try {
            const issues = [];
            ...
            await this.logOperation('validate', issues.length === 0 ? 'success' : 'warning', `验证配置文件: ${configPath} - ${issues.length} 个问题`);
            return result;
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : '未知错误';
    ...
    async logOperation(operation, status, message) {
        try {
            await this.ensureBackupDir();
            const logEntry = {
                timestamp: new Date().toISOString(),
